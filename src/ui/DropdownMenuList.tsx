...
export const DropdownMenuList: React.FC<DropdownMenuListProps> = ({
  ...
  <div
    ...
    className="rounded-[12px] border border-[#EAEDF0] bg-white shadow-[0_4px_24px_0px_rgba(37,45,52,0.10)] p-0 w-[228px]"
  >
    ...
    <ul className="max-h-[320px] overflow-y-auto pb-3" style={{ paddingLeft: 12, paddingRight: 8 }}>
      {items.length === 0 && (
        <li className="px-3 py-2 text-[#5F6E7C] text-[14px]">{notFoundText}</li>
      )}
      {[...items.filter((item) => !item.disabled), ...items.filter((item) => item.disabled)].map(
        (item) => (
          <li
            key={item.key}
          ...
          >
            <span className="truncate text-left" style={{ width: 160, marginLeft: 0 }}>
              {item.label}
            </span>
            {!item.disabled && (
              <Checkbox.Root
